  ...
  const getButtonStyle = () => {
    switch (type) {
      ...
      case buttonTypeKeys.APPLY:
        return `w-full py-4 flex justify-center items-center rounded-lg bg-neutral-100 bg-cover bg-center button-16-semibold text-neutral-100`;
  ...
  return (
    <>
      ...
      {type === buttonTypeKeys.SCRAP ? (
        ...
        // 스크랩 버튼 이외 텍스트 버튼
        <button
          className={`${getButtonStyle()} ${bgColor} ${fontColor}`}
          onClick={onClick}
        >
          {isCallIcon ? (
